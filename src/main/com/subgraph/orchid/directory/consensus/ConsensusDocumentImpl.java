...
public class ConsensusDocumentImpl implements ConsensusDocument {
    ...
    private final Set<String> serverVersions;
    @SuppressWarnings("MismatchedQueryAndUpdateOfCollection")
    private final Set<String> knownFlags;
    private HexDigest signingHash;
    private HexDigest signingHash256;
    private final Map<HexDigest, VoteAuthorityEntry> voteAuthorityEntries;
    private final List<RouterStatus> routerStatusEntries;
    private final Map<String, Integer> bandwidthWeights;
...
//        final KeyCertificate certificate = trustedAuthority.getCertificateByFingerprint(signature.getSigningKeyDigest());
    ...
    private boolean getBooleanParameterValue() {
        if(!parameters.containsKey(ConsensusDocumentImpl.USE_NTOR_HANDSHAKE_PARAM)) {
            return false;
        }
        final int value = parameters.get(ConsensusDocumentImpl.USE_NTOR_HANDSHAKE_PARAM);
        return value != 0;
    }
