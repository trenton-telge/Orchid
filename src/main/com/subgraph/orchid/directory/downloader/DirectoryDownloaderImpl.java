...
public class DirectoryDownloaderImpl implements DirectoryDownloader {
    private static final Logger logger = Logger.getInstance(DirectoryDownloaderImpl.class);
    ...
    public synchronized void start(Directory directory) {
        if(isStarted) {
            logger.warn("Directory downloader already running");
            return;
        }
    ...
    private <T extends Descriptor> List<T> removeUnrequestedDescriptors(Set<HexDigest> requested, List<T> received) {
        ...
        if(unrequestedCount > 0) {
            logger.warn("Discarding "+ unrequestedCount + " received descriptor(s) with fingerprints that did not match requested descriptors");
        }
        return result;
    }
    ...
    private DirectoryCircuit openBridgeCircuit(Router bridge) throws DirectoryRequestFailedException {
        try {
            return circuitManager.openDirectoryCircuitTo(Collections.singletonList(bridge));
        } catch (OpenFailedException e) {
