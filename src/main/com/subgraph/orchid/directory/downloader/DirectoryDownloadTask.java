...
public class DirectoryDownloadTask implements Runnable {
    private static final Logger logger = Logger.getInstance(DirectoryDownloadTask.class);
    ...
    private class DownloadConsensusTask implements Runnable {
        public void run() {
            try {
                ...
                logger.warn("Failed to download current consensus document: "+ e.getMessage());
            } finally {
    ...
    private class DownloadRouterDescriptorsTask implements Runnable {
        ...
        public void run() {
            try {
                ...
                logger.warn("Failed to download router descriptors: "+ e.getMessage());
            } finally {
    ...
    private class DownloadCertificatesTask implements Runnable {
        public void run() {
            try {
                ...
                logger.warn("Failed to download key certificates: "+ e.getMessage(), e);
            } finally {
