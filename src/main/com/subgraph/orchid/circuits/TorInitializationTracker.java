...
public class TorInitializationTracker {
    private static final Logger logger = Logger.getInstance(TorInitializationTracker.class);
    private final static Map<Integer, String> messageMap = new HashMap<>();
    ...
    private void notifyListeners(int code) {
        ...
        for(TorInitializationListener listener: getListeners()) {
            try {
                ...
                logger.fatal("Exception occurred in TorInitializationListener callback: "+ e.getMessage(), e);
            }
        }
    }

    private String getMessageForCode(int code) {
        return messageMap.getOrDefault(code, "Unknown state");
    }
