...
public class CircuitBuildTask implements Runnable {
    private static final Logger logger = Logger.getInstance(CircuitBuildTask.class);
    ...
    public void run() {
        ...
        try {
            ...
            logger.debug("Opening a new circuit to "+ pathToString(creationRequest));
        ...
        } catch(PathSelectionFailedException | TorException e) {
            circuitBuildFailed(e.getMessage());
        } catch(Exception e) {
            circuitBuildFailed("Unexpected exception: "+ e);
            logger.warn("Unexpected exception while building circuit: "+ e, e);
        }
    }
    ...
    private void notifyInitialization() {
        if(initializationTracker != null) {
            final int event = creationRequest.isDirectoryCircuit()?Tor.BOOTSTRAP_STATUS_ONEHOP_CREATE:Tor.BOOTSTRAP_STATUS_CIRCUIT_CREATE;
