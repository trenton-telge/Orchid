...
public class EntryGuards {
    private static final Logger logger = Logger.getInstance(EntryGuards.class);
    ...
    private List<Router> getMinimumUsableGuards(Set<Router> excluded) throws InterruptedException {
        synchronized(lock) {
            testStatusOfAllGuards();
            while(true) {
                List<Router> usableGuards = getUsableGuardRouters(excluded);
                if(usableGuards.size() >= EntryGuards.MIN_USABLE_GUARDS) {
                    return usableGuards;
                } else {
    ...
    private void initialProbeSucceeded(GuardEntry entry) {
        logger.debug("Probe connection to "+ entry.getRouterForEntry() + " succeeded.  Adding it as a new entry guard.");
    ...
    private Set<Router> getExcludedForChooseNew(Set<Router> excluded, List<Router> usable) {
        final Set<Router> set = new HashSet<>();
        set.addAll(excluded);
        set.addAll(usable);
    ...
    private void maybeChooseNew(int usableSize, Set<Router> excluded) {
        ...
        while(sz < EntryGuards.MIN_USABLE_GUARDS) {
            Router newGuard = chooseNewGuard(excluded);
            if(newGuard == null) {
                logger.warn("Need to add entry guards but no suitable guard routers are available");
                return;
            }
            logger.debug("Testing "+ newGuard + " as a new guard since we only have "+ usableSize + " usable guards");
