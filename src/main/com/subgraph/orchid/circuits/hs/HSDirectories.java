...
public class HSDirectories {
	...
	private final List<Router> hsDirectories;
	
	HSDirectories(Directory directory) {
		this.directory = directory;
		this.hsDirectories = new ArrayList<>();
		this.random = new TorRandom();
	}
	
	List<HSDescriptorDirectory> getDirectoriesForHiddenService(HiddenService hs) {
		final List<HSDescriptorDirectory> dirs = new ArrayList<>(2 * DIR_CLUSTER_SZ);
	...
	private List<Router> selectDirectoriesAtIndex(int idx) {
		...
		final List<Router> dirs = new ArrayList<>(DIR_CLUSTER_SZ);
