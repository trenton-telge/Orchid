...
public class ConnectionHandshakeV3 extends ConnectionHandshake {
	...
	void runHandshake() throws ConnectionIOException {
		sendVersions(3);
		receiveVersions();
		recvCerts();
	...
	void recvCerts() throws ConnectionHandshakeException  {
		...
		X509Certificate signing = null;
		X509Certificate tls = null;
		X509Certificate cross = null;
	...
	RSAPublicKey getConnectionPublicKey() {
		try {
			java.security.cert.X509Certificate[] chain = (X509Certificate[]) socket.getSession().getPeerCertificates();
	...
	public static boolean sessionSupportsHandshake(SSLSession session) {
		java.security.cert.X509Certificate cert = getConnectionCertificateFromSession(session);
		if(cert == null) {
			return false;
		}
	...
	static private java.security.cert.X509Certificate getConnectionCertificateFromSession(SSLSession session) {
		try {
			final java.security.cert.X509Certificate[] chain = (X509Certificate[]) session.getPeerCertificates();
			return chain[0];
		} catch (SSLPeerUnverifiedException e) {
			return null;
		}
	...
	static private boolean isSelfSigned(java.security.cert.X509Certificate certificate) {
		try {
	 ...
	 */
	static private boolean testModulusLength(java.security.cert.X509Certificate cert) {
